// Injector
(() => {
    globalThis["__tokio_internal_t"] = (() => {
        return {
            f: (f: string) => {
                return `[${f}]`
            },
            m: (m: string, c: string, e: string) => {
                var FLAG = false;
                let res = "";
            ...
            d: (r: Date) => {
                return `${r.getDate()}/${r.getMonth() + 1}/${r.getFullYear()}:${r.getHours()}:${r.getMinutes()}:${r.getSeconds()}.${r.getMilliseconds()}`
            },
            l: (l: string) => {
                let p = "";
                if (l == "ERR") {
    ...
    globalThis["__tokio_internal_format"] = (m: any, l: string, _f: string[], r: Date): string[] => {
        const message = new TraceMessage(r, m, l, _f);
        return [message.format("SIMPLE"), message.format("CORELOG"), message.format("TOPLOG")]
    };
    ...
    globalThis["__tokio_internal_msgformat"] = (t: Date, l: string, m: string, tc: string, ic: string, f?: "CORELOG" | "SIMPLE" | "TOPLOG"): string => {
        let time = globalThis["__tokio_internal_t"].d(t);
        let r = "";
        for (let i = 0; i < 23; i++) {
            if (time[i] == undefined) {
                r += " ";
