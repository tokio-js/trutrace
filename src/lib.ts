// Injector
(()=>{globalThis["__tokio_internal_t"]=(()=>{return{f:(f:string)=>{return`[${f}]`},m:(m:string,c:string,e:string)=>{var FLAG=false;let res="";for(let i=0;i<m.length;i++){if(m[i]=='"'&&FLAG==true){FLAG=false;res+=`${c}"`}else if(m[i]=='"'&&FLAG==false){FLAG=true;res+="\"\u001b[38;5;56m"}else{res+=m[i]}}return`${c}${res}${e}`},d:(r:Date)=>{return`${r.getDate()}/${r.getMonth()+1}/${r.getFullYear()}:${r.getHours()}:${r.getMinutes()}:${r.getSeconds()}.${r.getMilliseconds()}`},l:(l:string)=>{let p="";if(l=="ERR"){p="\u001b[38;5;196m"}else if(l=="WRN"){p="\u001b[38;5;208m"}else if(l=="INF"){p="\u001b[38;5;34m"}else if(l=="DBG"){p="\u001b[38;5;31m"}else if(l=="TRC"){p="\u001b[38;5;242m"}return[p,"\u001b[0m"]}}})();globalThis["__tokio_internal_format"]=(m:any,l:string,_f:string[],r:Date):string[]=>{const message=new TraceMessage(r,m,l,_f);return[message.format("SIMPLE"),message.format("CORELOG"),message.format("TOPLOG")]};globalThis["__tokio_internal_trace"]=(_i:number)=>{_i=Math.abs(Math.floor(_i));try{try{throw new Error()}catch(e){const arr:string[]=e.stack.replace(/  /g,"").split(/\n/g);const len=arr.length;let line:string="";let trc=arr[3+_i].split("(")[1].split(")")[0];for(let i=0;i<len;i++){if(arr[i].startsWith("at Object.<anonymous>")){line=arr[i].split("(")[1].split(")")[0];return[line,trc]}}return["",""]}}catch(e){if(e instanceof TypeError){try{throw new Error()}catch(e){const arr:string[]=e.stack.replace(/  /g,"").split(/\n/g);let trc=arr[4].split("at ")[1];return[trc,trc]}}else{console.error("Failed to trace, Error: "+e);return["",""]}}};globalThis["__tokio_internal_msgformat"]=(t:Date,l:string,m:string,tc:string,ic:string,f?:"CORELOG"|"SIMPLE"|"TOPLOG"):string=>{let time=globalThis["__tokio_internal_t"].d(t);let r="";for(let i=0;i<23;i++){if(time[i]==undefined){r+=" ";}else{r+=time[i];}}if(f == "CORELOG"){return`| ${r} | ${l} | ${globalThis["__tokio_internal_t"].f(ic)} ${m}`}else if(f=="TOPLOG"){return`| ${r} | ${l} | ${tc} ${m}`}else{let colors=globalThis["__tokio_internal_t"].l(l);return`\u001b[38;5;14m${globalThis["__tokio_internal_t"].d(t)} ${colors[0]}${l}${colors[1]} ${globalThis["__tokio_internal_t"].m(m,TraceMessage.TextColorStart,TraceMessage.TextColorEnd)}`}}})();
